        ...
        Partial Public Class Insert
            Public Shared Function UI_AddUser(ByVal name As String, ByVal username As String, ByVal pin As String, ByVal password As String, ByVal roleId As String, ByVal hasAgent As String, ByVal evoAgent As String) As String
                Try
                    Dim ReturnData As String = ""
                    Dim sqlConn As New SqlConnection(RTString)
                    Dim sqlComm As New SqlCommand("EXEC [dbo].[sp_UI_AddUser] @name, @username, @pin, @password, @roleId, @hasAgent, @evoAgent", sqlConn)
                    sqlComm.Parameters.Add(New SqlParameter("@name", name))
                    sqlComm.Parameters.Add(New SqlParameter("@username", username))
                    sqlComm.Parameters.Add(New SqlParameter("@pin", pin))
                    sqlComm.Parameters.Add(New SqlParameter("@password", password))
                    sqlComm.Parameters.Add(New SqlParameter("@roleId", roleId))
                    sqlComm.Parameters.Add(New SqlParameter("@hasAgent", hasAgent))
                    sqlComm.Parameters.Add(New SqlParameter("@evoAgent", evoAgent))
                    sqlConn.Open()
                    sqlComm.ExecuteNonQuery()
                    sqlComm.Dispose()
                    sqlConn.Close()
